...
import { ARTICULO_COMBINADO_EMPTY, ARTICULO_EMPTY, TipoArticulo, TipoEstado } from 'Config/util'
import { ARTICULOS_QUERY, ARTICULO_ID_QUERY, BORRAR_ARTICULO_QUERY, GUARDAR_ARTICULO_QUERY, MODIFICAR_ARTICULO_QUERY } from 'queries'
...
export function useArticulo({ id, tipo }: { id: string, tipo?: TipoArticulo }) {
    ...
    const { articulos, articulosAPI, usuario } = useAppSelector((state) => state)
    const [articuloQuery, artQueryResult] = useLazyQuery<{articulo: ArticuloDetallado}>(ARTICULO_ID_QUERY)
    const [modifyQuery, modQueryResult] = useMutation<{modificarArticulo: ArticuloDetallado}>(MODIFICAR_ARTICULO_QUERY)
    const [saveQuery, savQueryResult] = useMutation<{guardarArticulo: ArticuloDetallado}>(GUARDAR_ARTICULO_QUERY)
    const [deleteQuery, delQueryResult] = useMutation<{borrarArticulo: ArticuloDetallado}>(BORRAR_ARTICULO_QUERY)
    ...
    const getArticulo = () => {
        ...
        if (artAPI === undefined) {
            // console.log(`Hey ${id}`)
            articuloQuery({ variables: { id, tipo: TipoArticulo[tipoA] } }).then(({ data }) => {
                // console.log(data)
                if (data) {
                    articulo = {
                            ...data.articulo,
